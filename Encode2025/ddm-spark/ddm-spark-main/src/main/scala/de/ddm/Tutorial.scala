...
object Tutorial {
  ...
  def execute(spark: SparkSession): Unit = {
    ...
    employees.foreach(e =>
      if (e._4 == "Apple") appleAccumulator.add(1)
...
/*
    ...
    val animals = spark
      ...
      .option("quote", "\"")
      .option("delimiter", ",")
      .csv(s"data/animals.csv")
      .toDF("ID", "Name", "Species", "Weight", "Size", "DNA", "Prey")
      .as[(Int, String, String, Int, Int, String, String)]
    ...
    val result = animals
      .filter(t => t._5 > 100)
      .groupByKey(t => t._3)
      .mapGroups{ case (key, iterator) => (key, iterator.size) }

    //SELECT A.Name AS Predator, COUNT(B.Name) AS Preys
    //FROM animal A JOIN animal B ON A.Prey = B.ID
    //WHERE B.Size < 100
    //AND A.Size > 200
    //GROUP BY A.Name
    //ORDER BY A.Name;
    ...
    val B = animals
    val result2 = A
      .joinWith(B, A.col("Prey") === B.col("ID"))
      .filter(t => (t._1._5 > 200) && (t._2._5 < 100))
